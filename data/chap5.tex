...
\subsection{Dealing with the device variability Problem}
...
\emph{However, the chip area constrains the size of resistors.
...
\begin{figure}[!htbp]
    ...
    \label{fig:TIA_sch}
\end{figure}
\emph{Fig.\ref{fig:TIA_sch} shows the Transimpedance Amplifier circuit.
...
\emph{Fig.\ref{fig:TIA_postSim_dc} and Fig.\ref{fig:TIA_postSim_ac} are the dc and ac post-simulation of the TIA.
Fig.\ref{fig:TIA_postSim_dc} shows that the TIA has a constant transimpedance of $100k \Omega$ when the input current range is $6\mu A \sim -10\mu A$.
Fig.\ref{fig:TIA_postSim_ac}(a) indicates that the bandwidth is $7M Hz$.
Fig.\ref{fig:TIA_postSim_ac}(b) is the input referred noise (referred to the gate of nanowire).
...
\emph{The OpAmp of TIA holds the gain of 1000 (60dB), which makes the $Z_{in}$ of 100.
...
Thus, the maximal input impedance of the feedback circuit is $6k \Omega$ ($\frac{30M \Omega}{5k \Omega}$).
...
\emph{We swept Ibias and measured the output voltage of the feedback OP which is also the gate voltage of SiNW in Fig.\ref{fig:DCmode}.
...
\begin{figure}[!htbp]
    ...
    \caption{Post-simulation result of the dc sweep of Ibias in Fig.\ref{fig:DCmode}. (a) is the $I_D$-$V_G$ curves of $I_D$ and $I_bias$. (b) is the $g_m$-$V_G$ curves of intrinsic $g_m$ and measured $g_m$. (c) is the ratio of the difference between two $g_m$s (error).}
...
\emph{Table.\ref{tb:DCpostComp} compares the design specification and the simulation result.
It is notable that in fact the upper limits of $g_m$ is depends on two factors.
One is the gate voltage.
Table.\ref{tb:OPsim} shows that the maximal gate voltage that the Feedback OP can provides is $2.6V$.
The other is the current $I_bias$.
$I_bias$ is generated by a simple pmos whose current is decided by an external resistor (Fig.\ref{fig:mirror}).
...
According to our electrical measurement, this is the maximal $g_m$ value that our nanowire can have when $V_G$ is $2.6V$.}
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{l|c|c}
        ...
        \hline
        $g_m$ & $200n A$ - $30\mu A$& $130n A$ - $50\mu A$\\
    \end{tabular}
    \caption{The comparison between the design specification and simulation result of the DC-sweep mode circuit.}
    \label{tb:DCpostComp}
    }
\end{table}
...
It is notable that the second stage circuit is only used for the Transient Measurement.
...
\emph{In Fig.\ref{fig:sim:subtractor}(a), the result of ff corner exhibit poor upper input dynamic range. }
...
\emph{One thing to note is that the derivation above views the $V_z$ as the virtual ground.
$V_z$ is both the input and output offset voltage of the amplifier.
It is usually applied with $1.3V$.
...
\begin{figure}[!htbp]
    \begin{minipage}[t]{1\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth] {images/chapter5/postSim/ResAmp_1x.jpg}
        \raggedleft
        (a)
    \end{minipage}
    \hspace{8cm}
    \begin{minipage}[t]{1\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth] {images/chapter5/postSim/ResAmp_10x.jpg}
        \raggedleft
        (b)
    \end{minipage}
    \caption*{}
...
\begin{figure}[!htbp]
    \begin{minipage}[t]{1\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth] {images/chapter5/postSim/ResAmp_100x.jpg}
        \raggedleft
        (c)
    \end{minipage}
...
Fig.\ref{fig:sim:vgGain} presents the maximal gain that the circuit provides at five corners.
And Fig.\ref{fig:sim:vgnoise} shows the input referred noise.
...
\begin{table}[!htbp]
    ...
    \end{tabular}
    \caption{The summary table. The simulation results are compared with the design specification.}
    \label{tb:sim:vg}
    }
\end{table}
...
\emph{These results show that the circuit bandwidth is about $10k Hz$, which is caused by the last amplifier block.}
...
\begin{figure}[!htbp]
    ...
    \caption{The transient analysis. The gain of $A_{amp}$ is 100. The $g_m$ is swept from $1\mu$ to $6\mu$. There are two curves have the output saturation problem. This is because of the offset current flowing through $R_{TIA}$. One may solve it by increasing the current provided by Ibias.}
