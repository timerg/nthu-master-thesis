...
\begin{table}[!hbt]
    ...
    \begin{tabular}{l|cp{4cm}}
        ...
        C meas error & < 3\% & < 3\%\\
        \hline
        Detection of Input Current Varinace &  $120\mu A \sim 0.12n A$ & $850n A \sim 0.24n A$ \\
        \hline
        SNR & > 45dB & - \\
        \hline
...
The resistance measuring range of \cite{Juv2} is $1M \Omega \sim 1G \Omega$.
...
As for \cite{Jlockin}, the measuring range relates to the noise.
The noise referred to the input is $190n V$ and the resistance measuring range is $10 \Omega \sim 40k \Omega$.
Hence the input current noise of the circuit ranges from $19n A$ to $4.7p A$.
Moreover, the noise value is equivalent to a 2\% resistance resolution.
We therefore realize the current variance that the circuit can detect should be $850n A \sim 0.235n A$.

The relatively narrow current detection range of Method 2 should be because of the TIA block it adopts.
Our method adopts this TIA block and will discuss this problme in Section.\ref{sec:Ibias}.
...
\emphy{The gate coupling effect is illustrated in Fig.\ref{fig:gateCouple}.
The $\kappa$ is the ratio of voltage divider:
\begin{equation}
    \kappa = \frac{C_i}{C_i + C_{dep}}
\end{equation}
The fact that $C_{dep}$ varies with $V_G$ make $\kappa$ be a anon-linear parameter.
Its value ranges from 0.4 to 0.9.
For our nanowire, the structure is different from MOSFET.
...
Based on the experience in MOSFET, the gate coupling effect is not destructive.
...
\begin{figure}[bt!h]
    \centering
    \includegraphics[width=0.8\linewidth]{images/chapter6/kappa.png}
